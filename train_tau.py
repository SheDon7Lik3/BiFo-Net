...
def get_model_by_name(model_name, **kwargs):
    ...
    # dynamically import the requested model module
...
def train(config):
    ...
    # DIR augmentation: specify impulse response path and application probability
    train_ds = get_training_set(config.subset, device=None, roll=roll_samples,
                              apply_dir=config.ir_path is not None,  # enable DIR only when an IR path is provided
                              dir_p=config.dir_p,
...
if __name__ == '__main__':
    ...
    # Model selection - additional arguments
    ...
    parser.add_argument("--base_channels", type=int, default=24,
                       help="Base channel size; set to 36 for *_plus models")
    ...
    parser.add_argument('--dir_p', type=float, default=0.4)
    parser.add_argument('--ir_path', type=str, default="/content/ir_files")
